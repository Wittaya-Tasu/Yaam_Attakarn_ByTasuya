(() => {
  ...
  const els = {
    ...
    nowPrediction: document.getElementById('nowPrediction'), nowPredictionDot: document.getElementById('nowPredictionDot'),
    nowPredictionLabel: document.getElementById('nowPredictionLabel'), nowPredictionText: document.getElementById('nowPredictionText'),
  ...
  function render({ shouldScroll = false } = {}) {
    ...
    els.nowPrediction.className = `now-prediction level-${p.level}`;
    els.nowPredictionDot.className = `prediction-dot ${p.level}`;
    els.nowPredictionLabel.textContent = `ความหมาย • ${categoryLabels[selectedCategory]}`;
    els.nowPredictionText.textContent = p.text;
    ...
    if (shouldScroll) {
      requestAnimationFrame(() => {
        document.getElementById(`card-${currentId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      });
    }
